...
public class WoodenCogwheelBlockEntityRenderer extends KineticBlockEntityRenderer<BracketedKineticBlockEntity> {
	...
	protected void renderSafe(BracketedKineticBlockEntity be, float partialTicks, PoseStack ms,
		...
		if (EncasedPartialModels.SHAFTLESS_LARGE_COGS_MODELS.get(ForgeRegistries.BLOCKS.getKey(be.getBlockState().getBlock()).getPath().replaceAll("_large_cogwheel","")) == null) return;
		renderRotatingBuffer(be,
			CachedBuffers.partialFacingVertical(EncasedPartialModels.SHAFTLESS_LARGE_COGS_MODELS.get(ForgeRegistries.BLOCKS.getKey(be.getBlockState().getBlock()).getPath().replaceAll("_large_cogwheel","")), be.getBlockState(), facing),
