...
public class EncasedCustomCogRenderer extends KineticBlockEntityRenderer<SimpleKineticBlockEntity> {
	...
	protected SuperByteBuffer getRotatedModel(SimpleKineticBlockEntity be, BlockState state) {
		return CachedBuffers.partialFacingVertical(
			large ? EncasedPartialModels.SHAFTLESS_LARGE_COGS_MODELS.get(ForgeRegistries.BLOCKS.getKey(((EncasedCustomCogwheelBlock)state.getBlock()).getCogwheel().get()).getPath().replaceAll("_large_cogwheel","")) : EncasedPartialModels.COGS_MODELS.get(ForgeRegistries.BLOCKS.getKey(((EncasedCustomCogwheelBlock)state.getBlock()).getCogwheel().get()).getPath().replaceAll("_cogwheel","")), state,
